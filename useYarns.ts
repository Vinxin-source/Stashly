...
export function useYarns() {
  ...
  const deleteYarn = async (id: string) => {
    try {
      const supabase = createClient();
      const { error: deleteError } = await supabase.from("yarns").delete().eq("id", id);
      if (deleteError) {
  ...
  return { yarns, loading, error, addYarn, deleteYarn, refresh: fetchYarns };
}
